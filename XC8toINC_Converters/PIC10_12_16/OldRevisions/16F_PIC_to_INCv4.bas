...
#DEFINE kINDEX_IDX      "C:\Users\admin\.mchp_packs"
...
Dim Shared SourceFileArray(1000000) As SourceFiles
...
Dim Shared SFRBits(200000) as SFRBits
Dim Shared SFRUniques() as string
Dim Shared currentitem as string
Dim Shared BadRam(10000) as string
...
Declare Sub PopulateConfig ( SourceFolder as string, Version as string, Chip as string )
  ...
  Else
    ...
    End if
    if ShowDebug = -1 then print ";Debug: kINDEX_IDX directory location is: "+kINDEX_IDX
    ...
    fsp_ini = kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\pic\dat\ini\"+ chip  +".ini"
    ConfigFilename = "; " + kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\pic\dat\cfgdata\"+ Chip  +".cfgdata"
    ...
    Print "; " + fsp_ini
    print "; " + kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\edc\"+ Chip  +".PIC"
    print "; " + kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\pic\dat\cfgdata\"+ Chip  +".cfgdata"
    print ";"
    Print ";=========================================================================="
    ...
    do while not eof(1)
        ...
        If DataSource <> "" and left( datasource, 4 ) = "SFR=" Then
            ...
            if FoundRegistersToBeIgnored = 0 then
                ...
                print "*"+SFRData(SFRDataCount).RegisterName+"*"
                split( SFRData(SFRDataCount).RegisterName , "," , 99 , SFRRegister() )
                'with SFRData(SFRDataCount)

                    SFRData(SFRDataCount).RegisterName = SFRRegister(0)
                    SFRData(SFRDataCount).RegisterAddresss = SFRRegister(1)
                    SFRData(SFRDataCount).RegisterLength = SFRRegister(2)

                    ThisBank = int(val("&h"+trim(SFRRegister(1)))/127)
                    if ( ThisBank <>  CurrentBank ) then
                        CurrentBank = ThisBank
                        print ""
                        print ";-----Bank"+str(CurrentBank)+"------------------"
                    end if
                    print left(SFRData(SFRDataCount).RegisterName+"                 ",17)+"EQU  H'"+right("000"+SFRRegister(1),4)+"'"
                'end with
            end if
        end if
    loop
...
'        End Type
    ...
    '    loop until SFRBits(SFRBitsArrayPointer).Count = 0
        ...
        PopulateConfig ( chipparameters(3), chipparameters(1), chip )
...
Sub PopulateSourceFilesLocation
  ...
  xmldoc->load( kINDEX_IDX+"\index.idx" )
