...
#DEFINE kINDEX_IDX  "C:\Program Files\Microchip\MPLABX\v6.15\packs"  ' Environ( "USERPROFILE" )+"\.mchp_packs"

                     'the version may need changing 
#DEFINE kINDEX_IDX2  "C:\Program Files\Microchip\MPLABX\v6.15\packs"
...
PopulateSourceFilesLocation  ( kINDEX_IDX2 )
  ...
  If COMMAND(CD) <> "" then
    ...
    Print ";=========================================================================="
    Print ";"
    Print ";  Built by GCBASIC convertor : " + Command(0)
        ...
        If DataSource <> "" and left( datasource, 7 ) = "SFRFLD=" Then
            ...
            split ( DataSource, ",", 99, RegisterBits() )
              ...
              if RegisterBits(1) <> CurrentRegister then
                  'a new register? New line, new construct
              ...
              if  trim(RegisterBits(3)) = "1" then
                ...
                if trim(RegisterBits(3)) <> "8" Then 
                  Dim indexBit as Integer
                  For indexBit = 0 to val(trim(RegisterBits(3)))-1
                    SFRBitsArrayPointer+=1
                    'Walk thru the bits ( clearly more than one), and, a the numeric suffix
                    SFRBits(SFRBitsArrayPointer).RegisterRaw = left(RegisterBits(0)+Str(indexbit)+"                ",17)+"EQU  H'"+right("0000"+Str(Val(RegisterBits(2))+indexbit),4)+"'"
                    SFRBits(SFRBitsArrayPointer).RegisterBitName = RegisterBits(0)+Str(indexbit)
                    SFRBits(SFRBitsArrayPointer).RegisterBitAddress = Str(Val(RegisterBits(2))+indexbit)
                    SFRBits(SFRBitsArrayPointer).Count = SFRBitsArrayPointer
                    SFRBits(SFRBitsArrayPointer).RegisterName = RegisterString(CurrentRegister)
                  Next
                else
                  ' must be 8 bits and that is not valid as 8 bits is a register
                end If
              end if
        End if
    loop
...
'            CVALUE:70:EXTOSC:EXTOSC operating per FEXTOSC bits (device manufacturing default)
    ...
    do while not eof(1)
        ...
        if left(ucase(datasource),6)="CWORD:" and instr(datasource,"CONFIG")<>0 then
            ...
            If dumpconfig = 0 then
              ' may need to add CONFIG11 or above handler for 18FxxQ8x chips
              If Instr(Ucase(Chip), "Q8") > 0 Then
                If Instr(Ucase(configname), "CONFIG11") Then
                  dumpconfig = -1
                End iF
              End If
            end if
