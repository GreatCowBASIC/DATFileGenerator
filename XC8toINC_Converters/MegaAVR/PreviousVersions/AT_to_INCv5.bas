...
Declare Sub PopulateConfig ( SourceFolder as string, Version as string, Chip as string )
  ...
  Else
    ...
    Dim As String chip, chipdetails, chiphsource
    ...
    fsp_ini = UserProfile+kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\avr\include\avr\"+ chiphsource  +".h"
    ...
    print "; " + UserProfile + kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\avr\dat\cfgdata\"+ Chip  +".cfgdata"
    print ";"
    Print ";=========================================================================="
...
'    Print "        ENDIF"
    ...
    do while not eof(1)
        ...
        If DataSource <> "" and instr( datasource, "_SFR_" ) > 0 and  instr( datasource, "MEM16" )= 0  Then
            ...
            if FoundRegistersToBeIgnored = 0 then
                ...
                SFRData(SFRDataCount).RegisterName = trim(mid(Trim(UCase(DataSource)),9, instr(DataSource, "_" )-10 ))
                with SFRData(SFRDataCount)
                    .RegisterAddresss = mid(DataSource, instr( DataSource,"(" )+1 , instr( DataSource,")")-instr( DataSource,"(")-1  )
                    'Output register
...
'        End Type
    ...
    Dim SlashPosition as Integer
        ...
        do while not eof(1)
            ...
            if instr( datasource, "Interrupt Vectors" ) > 0 then exit do

            If DataSource <> "" and instr( datasource, "_SFR_" ) > 0 and instr( datasource, "#define" ) > 0 Then
                 'cache register
                  NewRegister = trim(mid(Trim(UCase(DataSource)),9, instr(DataSource, "_" )-10 ))
                  readNextLine = -1
                  ...
                  if NewRegister <> CurrentRegister then
                      ...
                      Do
                          ...
                          if instr( datasource, "#define" ) = 0 then Exit Do

                          if instr( datasource, "_SFR_" ) = 0  then
                              replace( DataSource,"#define","")
                              DataSource = Trim( DataSource )
                              SpacePosition = Instr ( DataSource, " " )
                              SlashPosition = Instr ( DataSource, "/" )
...
'                              print "<"+DataSource+">",RegisterBits(0),RegisterBits(1)+"*"
                              ...
                              SFRBits(SFRBitsArrayPointer).RegisterBitName = trim( Left( DataSource, SpacePosition ) )

                              SFRBits(SFRBitsArrayPointer).RegisterBitAddress = trim( Mid( DataSource, SpacePosition, SlashPosition-SpacePosition-1 ) )
        ...
        Do while not eof ( 1 )
            ...
            Case 26
                 Print "Usart0Start:USART0_Start,52,SFDE,"
            Case 27
                 Print "PinChange3:PCINT3,54,PCIE3,PCIF3"
            Case 28
                 Print "UsartRX1Ready:USART1_RX,56,RXCIE1,RXC1"
            Case 29
                 Print "UsartTX1Sent:USART1_TX,58,TXCIE1,TXC1"
            Case 30
                 Print "UsartTX1Ready:USART1_UDRE,60,UDRIE1,UDRE1"
            Case 31
                 Print "Usart1Start:USART1_Start,62,SFDE1,"
            Case 32
