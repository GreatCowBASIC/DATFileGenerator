...
'This examines the INDEX_IDX file in the MPLAB-X directory and returns the version etc for a specific Chip
  ...
  Dim Shared FindOnly as Integer = 0
...
InitAndGetFiles
  ...
  if FindOnly then 
    If left(targetchip,3) = "PIC" then
      print UserProfile+kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\pic\include\proc\"+ chipincsource  +".inc"
    Else
      print UserProfile+kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\avrasm\inc\"+ chipincsource  +".inc"
    End If
    End
  End if
...
Function FindChip ( chiptofind as string, ignore as string ) as string
    ...
    for loopcounter = 0 to SourceFileArrayPointer - 1
      ' print SourceFileArray( loopcounter ).ChipName, trim(ucase(SourceFileArray( loopcounter ).ChipName)) = trim(ucase(chiptofind))
      ...
      with SourceFileArray( loopcounter )
          if  ucase(trim( .ChipName )) = ucase(trim(chiptofind)) then
              ' print loopcounter,SourceFileArray( loopcounter ).ChipName, trim(ucase(SourceFileArray( loopcounter ).ChipName)) = trim(ucase(chiptofind))
...
Sub InitAndGetFiles
  ...
  UserProfile = UserProfile + "\.mchp_packs"
    ...
    UserProfile = UserProfile + "\.mchp_packs"
      ...
      dim seenvalidrecord as Integer = 0
      ...
      For forIndex = 1 to 2 
          Do
            ...
            if left(chipdetails,6) = "NOCHIP" or  chipdetails = "" then
              if forindex = 2 and ShowIncludeLocation = 0 and seenvalidrecord =  0 then 
                print "Not a valid chip, or no DFP files .. ", chipdetails
                print forIndex, fsp_incfile
                End
              End if
              exit do
            else
              ' print chipdetails
              If seenvalidrecord = 0 then seenvalidrecord = -1
            end if
            ...
            If left(targetchip,3) = "PIC" then
              fsp_incfile = UserProfile+kINDEX_IDX+"\Microchip\"+chipparameters(3)+"_DFP\"+chipparameters(1)+"\xc8\pic\include\proc\"+ chipincsource  +".inc"
            Else
